(function() {
  ...
  async function changePassword() {
    ...
    try {
      var data = await api({ action: 'updateUserPass', password: adminPassword, index: idx, newPass: newPass });
  ...
  async function addUser() {
    ...
    try {
      var data = await api({ action: 'addUser', password: adminPassword, name: name, pass: pass, role: role, perms: perms.join(',') });
      if (data && data.success) {
  ...
  async function deleteUser(i, name) {
    ...
    try {
      var data = await api({ action: 'deleteUser', password: adminPassword, index: i });
      if (data && data.success) {
